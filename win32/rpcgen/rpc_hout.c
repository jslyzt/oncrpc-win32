...
void pconstdef(definition* def) {
    pdefine(def->def_name, def->def.co);
}

void puldefine(char* name, char* num) {
    f_print(fout, "#define %s ((u_long)%s)\n", name, num);
}

int define_printed(proc_list* stop, version_list* start) {
    version_list* vers;
    proc_list* proc;
...
void pprogramdef(definition* def) {
    version_list* vers;
    proc_list* proc;
...
int undefined2(char* type, char* stop) {
    list* l;
    definition* def;
...
void pstructdef(definition* def) {
    decl_list* l;
    char* name = def->def_name;
