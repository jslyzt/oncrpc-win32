 ...
 */
CLIENT* clnttcp_create(struct sockaddr_in* raddr, u_long prog, u_long vers, register int* sockp, u_int sendsz, u_int recvsz) {
    CLIENT* h;
...
static enum clnt_stat clnttcp_call(register CLIENT* h, u_long proc, xdrproc_t xdr_args, caddr_t args_ptr, xdrproc_t xdr_results, caddr_t results_ptr, struct timeval timeout) {
...
static int readtcp(register struct ct_data* ct, caddr_t buf, register int len) {
    ...
    return (len);
#else
        switch (select(_rpc_dtablesize(), &readfds, (int*)NULL, (int*)NULL, &(ct->ct_wait))) {
            case 0:
                ct->ct_error.re_status = RPC_TIMEDOUT;
                return (-1);
            case -1:
